...
const AI_IAMGES_PATH = path.join(
    __dirname,
    '..',
    '..',
    '..',
...
export const iamgeGenerationUseCase = async (
    ai: GoogleGenAI, imageGenerationDto: ImageGenerationDto,
    options?: Options
): Promise<ImageGenerationResponse> => {
    const { prompt, files = [] } = imageGenerationDto;
    const contents: ContentListUnion = [{ text: prompt }];
    const filesUploaded = await geminiUploadFiles(ai, files, { transformToPng: true });
    ...
    for (const part of response.candidates?.[0]?.content?.parts ?? []) {
        ...
        const imagePath = path.join(AI_IAMGES_PATH, `${imageId}.png`);
